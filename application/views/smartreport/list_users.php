		...
		<!-- Row toggler -->
				<div class="card">
					...
					<div class="form-group row">
						...
						<div class="col-lg-8">
							<div class="input-group">
								...
								<form action="<?php echo site_url('smartreport/list_users'); ?>" class="form-inline" method="get">
									...
									<span class="input-group-append">									
										...
										<?php if ($q <> ''){?>
											<a href="<?php echo site_url('smartreport/list_users'); ?>" class="btn btn-light">Reset</a>
										<?php } ?>
									</span>
								</form>
							</div>										
					...
					<table class="table table-bordered table-togglable table-hover  ">
						...
						<tbody>
						<?php
							foreach ($smartreport_users_data as $smartreport_users){?>
							<tr>
								...
								<?php }else if ($smartreport_users->user_status === '1'){ ?>
									...
									<div class="list-icons">
										<div class="dropdown">
											...
											<div class="dropdown-menu dropdown-menu-right">
												<a data-toggle="modal" data-target="#modal_edit_user<?=$smartreport_users->iduser;?>" class="dropdown-item"><i class="icon-pencil"></i><?php echo $lang_edit_user; ?></a>
												<a data-toggle="modal" data-target="#modal_change_password<?=$smartreport_users->iduser;?>"class="dropdown-item"><i class="icon-key"></i><?php echo $lang_change_password; ?></a>
												<a href="<?php echo base_url('smartreport/delete-user/'.$smartreport_users->iduser);?>" class="dropdown-item delete_data"><i class="icon-cross2"></i><?php echo $lang_delete_user; ?></a>
											</div>
										</div>
									</div>
								</td>
				...
				<?php foreach ($smartreport_users_data as $smartreport_users){?>
				<div id="modal_change_password<?=$smartreport_users->iduser;?>" class="modal fade" tabindex="-1"  aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title"><?php echo $lang_change_password_for; ?> <?=$smartreport_users->user_name;?></h5>
								<button type="button" class="close"  data-dismiss="modal" aria-hidden="true">&times;</button>
							</div>

							<form action="<?=base_url()?>smartreport/update_password_user" method="post">
								<div class="modal-body">
								<input type="hidden" name="iduser" value="<?=$smartreport_users->iduser;?>" required>
				...
				<div id="modal_add_user" class="modal fade" tabindex="-1" aria-hidden="true">
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<form action="<?=base_url()?>smartreport/insert_user" method="post">
								<div class="modal-body">
									<div class="form-group">
										<div class="row">
											<div class="col-sm-6">
									...
									<div class="form-group">
										<div class="row">
											...
											<div class="col-sm-6">
												...
												<?php
													$deptData = $this->Smartreport_users_model->getDataAll('smartreport_dept', 'iddept', 'ASC');
				...
				<!-- Vertical form modal -->
				<?php foreach ($smartreport_users_data as $smartreport_users){?>
				<div id="modal_edit_user<?=$smartreport_users->iduser;?>" class="modal fade" tabindex="-1"  aria-hidden="true">
				<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
							...
							<form action="<?=base_url()?>smartreport/update_user" method="post">
								<div class="modal-body">
									<div class="form-group">
										<div class="row">
											<div class="col-sm-6">
												<label><?php echo $lang_username; ?></label>
												<input type="text" name="user_name" placeholder="Username..." class="form-control" value="<?=$smartreport_users->user_name;?>" required>
											</div>

											<div class="col-sm-6">
												<label><?php echo $lang_email; ?></label>
												<input type="email" name="user_email" placeholder="Email..." value="<?=$smartreport_users->user_email;?>" class="form-control" required>
											</div>
										</div>
									</div>
										...
										<div class="row">		
											...
											<div class="col-sm-6">
												...
												<select name="iddept" class="form-control" required autocomplete="off">
													<option value=""><?php echo $lang_choose_departement; ?></option>
												<?php
													$departement = $smartreport_users->iddept;
													$deptData = $this->Smartreport_users_model->getDataAll('smartreport_dept', 'iddept', 'ASC');
													for ($p = 0; $p < count($deptData); ++$p) {
														$iddept = $deptData[$p]->iddept;
														$deptname = $deptData[$p]->deptname;?>
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
												...
												<select name="user_status" class="form-control" required autocomplete="off">
													...
													<option <?php if ($smartreport_users->user_status === '1') {echo 'selected="selected"';} ?> value="1"><?php echo $lang_active; ?></option>
													<option <?php if ($smartreport_users->user_status === '0') {echo 'selected="selected"';} ?>value="0"><?php echo $lang_inactive; ?></option>
												</select>
											</div>
										</div>
									</div>
								...
								<div class="modal-footer">
									<input type="hidden" name="iduser" class="form-control" value="<?=$smartreport_users->iduser;?>" required>
									<button type="button" class="btn btn-link" aria-hidden="true" data-dismiss="modal"><?php echo $lang_close; ?></button>
									<button type="submit" class="btn bg-primary"><?php echo $lang_submit; ?></button>
								</div>
							</form>
